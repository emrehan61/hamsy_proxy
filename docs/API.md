# rdproxy API reference

This document specifies the JSON wire format and REST/WebSocket surface actually implemented by `rdproxy-api` (`crates/rdproxy-api/src/`), reconciled against the source and the crate's own test suite (`crates/rdproxy-api/tests/rest_api.rs`, `tests/ws.rs`). Earlier drafts of this document described a few things `rdproxy-core`/`rdproxy-api` ended up implementing differently (noted inline below); this revision matches the real server.
...
The server is `rdproxy-api`'s `axum::Router` (`rdproxy_api::router`), served by `rdproxy run` on the configured UI port (default `9081`), or standalone with a `NoopReplay`/`StubCert` backend. There is **no authentication** on any of this — see the Security note in the top-level `README.md`.
...
`request`/`response` are `null` until that half of the flow has actually been received; `originalRequest`/`originalResponse` stay `null` unless `modified` is true. `tls` is `null` for plain HTTP flows. `peerCert*`/`notBefore`/`notAfter` on `tls` are currently always `null` in practice — rdproxy's hand-rolled ClientHello parser only extracts SNI/ALPN, not the peer certificate fields (`connect.rs`'s `build_tls_info`).
...
Note: `rdproxy-api` never actually constructs a `flowDetail` event itself today (no route/handler emits `ServerEvent::FlowDetail`); it exists in the wire protocol and is handled the same as any other event by the coalescing logic, but nothing currently sends one.
        ...
        body: optional edited RequestRecord as raw JSON (empty body ->
              replay the flow's own original request)
                     ...
                     -> 501 if the configured ReplayHook can't replay (e.g.
                        `rdproxy-api` running standalone with no proxy
                        backend attached; not reachable via `rdproxy run`,
...
POST   /api/system-proxy
        body: {"enabled": true}
                     ...
                     — via this same endpoint, `rdproxy proxy off`, or
                     `rdproxy run` shutting down — restores that prior
...
GET    /api/setup
                     -> 200 {
                          ...
                          "certUrl": "http://192.168.1.10:9081/cert/rdproxy-ca.crt",
