...
Changes made through the API (or `hamsy rules import`) take effect immediately on the next request — the compiled rule set is rebuilt on every mutation, there's no caching lag.
...
Each recipe was POSTed to a locally running `hamsy run` instance's `POST /api/rules` and returned `201 Created` — see "Verification" for the exact commands and responses.
...
```
cargo run -q -p hamsy-cli -- run --no-open --proxy-port 19082 --ui-port 19083 --data-dir <scratch-dir> &
