...
const Settings: Component = () => {
  ...
  const onExportHarAll = async () => {
    ...
    try {
      ...
      triggerDownload(blob, filename ?? "rdproxy-session.har");
    } catch {
  ...
  const onExportRules = async () => {
    try {
      ...
      triggerDownload(new Blob([JSON.stringify({ rules: exported }, null, 2)], { type: "application/json" }), "rdproxy-rules.json");
    } catch {
    ...
    <div class="settings-page">
      <Show when={settingsRestartRequired()}>
        <div class="settings-page__banner" role="status">
          <span>Restart rdproxy for the new port/bind-address to take effect.</span>
