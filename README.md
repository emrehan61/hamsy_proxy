...
- **Settings** (`/settings`) — grouped fields for proxy ports/bind address/upstream proxy, capture (pause, max flows, max body size, WS capture, include/exclude host globs), HTTPS interception (passthrough hosts), system proxy toggle, theme, and data management (HAR export/import, rules export/import, clear all flows). Each field is persisted with a debounced `PUT /api/settings` partial patch as you type.
...
- **Some app breaks entirely once the proxy/CA is on**, rather than just showing one failed request — almost always certificate pinning; see [Certificate-pinned apps will not work](#certificate-pinned-apps-will-not-work). The fix isn't on hamsy-proxy's side: add that app's host to `passthroughHosts` (Settings, or `settings.json` directly) so it's never MITM'd.
