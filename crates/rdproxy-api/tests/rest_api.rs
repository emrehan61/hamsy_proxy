...
async fn state_shape() {
    ...
    for key in ["version", "proxyPort", "uiPort", "capturing", "paused", "flowCount", "caFingerprint", "uptimeSecs", "systemProxy"] {
...
async fn flows_list_filters_and_after_seq() {
    let state = common::make_state();
    state.flows().insert(common::sample_flow(1, "GET", "a.com", Some(200)));
    state.flows().insert(common::sample_flow(2, "POST", "b.com", Some(404)));
    state.flows().insert(common::sample_flow(3, "GET", "a.com", Some(500)));
    let app = router(state);
    ...
    // Method filter.
    let resp = app.clone().oneshot(get("/api/flows?methods=POST")).await.unwrap();
    let body = body_json(resp).await;
    assert_eq!(body["flows"].as_array().unwrap().len(), 1);

    // Host filter.
    let resp = app.clone().oneshot(get("/api/flows?host=a.com")).await.unwrap();
    let body = body_json(resp).await;
    assert_eq!(body["flows"].as_array().unwrap().len(), 2);

    // statusClass filter.
    let resp = app.clone().oneshot(get("/api/flows?statusClass=4")).await.unwrap();
    let body = body_json(resp).await;
    assert_eq!(body["flows"].as_array().unwrap().len(), 1);

    // afterSeq + limit.
    let resp = app.clone().oneshot(get("/api/flows?afterSeq=1&limit=1")).await.unwrap();
    let body = body_json(resp).await;
    ...
    let resp = app.clone().oneshot(get("/api/flows?resourceTypes=script")).await.unwrap();
    let body = body_json(resp).await;
...
async fn flow_get_by_id_404_for_unknown_uuid() {
    ...
    let resp = app.oneshot(get(&format!("/api/flows/{random_id}"))).await.unwrap();
...
async fn replay_501_when_no_backend_attached() {
    ...
    let req = Request::builder().method("POST").uri(format!("/api/flows/{id}/replay")).body(Body::empty()).unwrap();
    let resp = app.oneshot(req).await.unwrap();
...
async fn rules_crud_toggle_reorder_round_trip() {
    ...
    let resp = app.clone().oneshot(json_req("POST", "/api/rules", sample_rule("", "first"))).await.unwrap();
    ...
    let resp = app.clone().oneshot(json_req("POST", "/api/rules", sample_rule("second-id", "second"))).await.unwrap();
    assert_eq!(resp.status(), StatusCode::CREATED);

    // Toggle flips enabled.
    let resp = app.clone().oneshot(Request::builder().method("POST").uri(format!("/api/rules/{id}/toggle")).body(Body::empty()).unwrap()).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK);
    ...
    let resp = app.clone().oneshot(json_req("PUT", &format!("/api/rules/{id}"), updated_body)).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK);
    let updated = body_json(resp).await;
    assert_eq!(updated["name"], "renamed");

    // Update on unknown id is 404.
    let resp = app.clone().oneshot(json_req("PUT", "/api/rules/nope", sample_rule("nope", "x"))).await.unwrap();
    assert_eq!(resp.status(), StatusCode::NOT_FOUND);

    // Reorder.
    let resp = app
        .clone()
        .oneshot(json_req("POST", "/api/rules/reorder", json!({"ids": ["second-id", id.clone()]})))
        .await
        .unwrap();
    assert_eq!(resp.status(), StatusCode::NO_CONTENT);
    let resp = app.clone().oneshot(get("/api/rules")).await.unwrap();
    ...
    // Delete.
    let resp = app.clone().oneshot(delete(&format!("/api/rules/{id}"))).await.unwrap();
    assert_eq!(resp.status(), StatusCode::NO_CONTENT);
    let resp = app.clone().oneshot(delete(&format!("/api/rules/{id}"))).await.unwrap();
    assert_eq!(resp.status(), StatusCode::NOT_FOUND);
...
async fn rules_list_returns_rules_wrapper_object() {
    ...
    let create_resp =
        app.clone().oneshot(json_req("POST", "/api/rules", sample_rule("r1", "one"))).await.unwrap();
    ...
    let obj = body.as_object().expect("must be a JSON object, not a bare array");
    assert_eq!(obj.keys().collect::<Vec<_>>(), vec!["rules"], "unexpected top-level keys: {body}");
...
async fn rules_create_without_id_field_gets_server_assigned_id() {
    ...
    assert!(body_without_id.get("id").is_none(), "test payload must omit `id` to exercise the bug");

    let resp = app.clone().oneshot(json_req("POST", "/api/rules", body_without_id)).await.unwrap();
    assert_eq!(resp.status(), StatusCode::CREATED);
    let created = body_json(resp).await;
...
async fn settings_partial_merge_and_restart_required() {
    ...
    let resp = app.clone().oneshot(json_req("PUT", "/api/settings", json!({"theme": "light"}))).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK);
    let body = body_json(resp).await;
    ...
    let resp = app.clone().oneshot(json_req("PUT", "/api/settings", json!({"proxyPort": 12345}))).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK);
    let body = body_json(resp).await;
...
async fn har_export_has_correct_headers_and_shape() {
    ...
    let disposition = resp.headers().get("content-disposition").unwrap().to_str().unwrap().to_string();
...
async fn har_import_increases_flow_count() {
    ...
    let resp = app.oneshot(json_req("POST", "/api/har/import", har_doc)).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK);
    let body = body_json(resp).await;
    assert_eq!(body["imported"], 1);
    assert_eq!(state.flows().len(), 1);
...
async fn setup_payload_has_expected_shape() {
    ...
    assert_eq!(resp.status(), StatusCode::OK);
    let body = body_json(resp).await;
    for key in ["proxyHost", "proxyPort", "lanAddresses", "certUrl", "caFingerprint", "qrSvg"] {
