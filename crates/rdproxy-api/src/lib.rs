//! REST + WebSocket API and web UI asset server for rdproxy.
//!
//! This crate is self-contained: it depends only on `rdproxy-core` for the
...
pub fn router(state: ApiState) -> Router {
    ...
    let cert_routes = Router::new()
        .route("/rdproxy-ca.pem", get(routes::setup::cert_pem))
        .route("/rdproxy-ca.crt", get(routes::setup::cert_crt))
        .route("/rdproxy-ca.der", get(routes::setup::cert_der))
        .fallback(api_not_found);
