...
//!
//! `rdproxy-api` is intentionally self-contained: it defines these small
...
impl CertHook for StubCert {
    fn ca_pem(&self) -> String {
        "-----BEGIN CERTIFICATE-----\n\
         PLACEHOLDER: no certificate authority attached to rdproxy-api\n\
         -----END CERTIFICATE-----\n"
            .to_string()
    }

    fn ca_der(&self) -> Vec<u8> {
        b"PLACEHOLDER: no certificate authority attached to rdproxy-api".to_vec()
    }
