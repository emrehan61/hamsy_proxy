//! Persisted marker recording that *rdproxy* (via any of its three
//! enable-the-system-proxy call sites: `rdproxy run` startup, the web
//! UI's `POST /api/system-proxy`, or `rdproxy proxy on`) is the reason the
//! OS system proxy currently points at this machine's rdproxy instance --
...
//! as it's there, *something* -- a still-running `rdproxy run`, or a
//! crashed one -- currently has the system proxy pointed at rdproxy and
...
//! web UI or another shell's `rdproxy proxy on`) and hard kills (nothing
//! in-process survives a SIGKILL to react to).
//!
//! If rdproxy is killed hard enough that no code in this process ever
...
//! SIGKILL, so the next `rdproxy` invocation that starts is the earliest
...
pub struct MarkerState {
    ...
    /// rdproxy enabled it -- what gets put back.
...
pub fn acquire(data_dir: &Path, host: &str, port: u16, bypass: &[String]) -> Result<(), String> {
    // If a previous rdproxy run left a marker behind (crash/kill without
...
/// off/restored, no matter who turned it on" -- `rdproxy proxy off` and
...
/// restore-on-shutdown/panic, where a plain `rdproxy run` that never
...
/// proxy. If a marker is present, a previous rdproxy process died without
...
pub fn recover_stale(data_dir: &Path) {
    ...
    tracing::warn!(
        ...
        host = %marker.host,
        port = marker.port,
        "restored system proxy settings left behind by a previous rdproxy run (pid {})",
        marker.pid
    );
    if let Err(err) = sysproxy::restore(&marker.snapshot) {
        tracing::warn!(%err, "failed to restore system proxy settings left behind by a previous rdproxy run");
    }
}
...
mod tests {
    ...
    /// `rdproxy-api/tests/common/mod.rs`'s `temp_path` pattern.
    fn temp_dir() -> PathBuf {
        std::env::temp_dir().join(format!(
            "rdproxy-sysproxy-state-test-{}",
            uuid::Uuid::new_v4()
        ))
    }
