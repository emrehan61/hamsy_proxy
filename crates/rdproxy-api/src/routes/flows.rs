...
use axum::Json;
use bytes::Bytes;
use rdproxy_core::{FlowQuery, RequestRecord, ResourceType, ServerEvent};
...
/// `GET /api/flows/:id` — returns the full [`rdproxy_core::Flow`] detail.
pub async fn get(
    State(state): State<ApiState>,
    Path(id): Path<String>,
) -> Result<Json<rdproxy_core::Flow>, ApiError> {
