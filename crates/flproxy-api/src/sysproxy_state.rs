//! Persisted marker recording that *flproxy* (via any of its three
//! enable-the-system-proxy call sites: `flproxy run` startup, the web
//! UI's `POST /api/system-proxy`, or `flproxy proxy on`) is the reason the
//! OS system proxy currently points at this machine's flproxy instance --
...
//! as it's there, *something* -- a still-running `flproxy run`, or a
//! crashed one -- currently has the system proxy pointed at flproxy and
...
//! web UI or another shell's `flproxy proxy on`) and hard kills (nothing
//! in-process survives a SIGKILL to react to).
//!
//! If flproxy is killed hard enough that no code in this process ever
...
//! SIGKILL, so the next `flproxy` invocation that starts is the earliest
...
pub struct MarkerState {
    ...
    /// flproxy enabled it -- what gets put back.
...
pub fn acquire(data_dir: &Path, host: &str, port: u16, bypass: &[String]) -> Result<(), String> {
    // If a previous flproxy run left a marker behind (crash/kill without
...
/// off/restored, no matter who turned it on" -- `flproxy proxy off` and
...
/// restore-on-shutdown/panic, where a plain `flproxy run` that never
...
/// proxy. If a marker is present, a previous flproxy process died without
...
pub fn recover_stale(data_dir: &Path) {
    ...
    tracing::warn!(
        ...
        host = %marker.host,
        port = marker.port,
        "restored system proxy settings left behind by a previous flproxy run (pid {})",
        marker.pid
    );
    if let Err(err) = sysproxy::restore(&marker.snapshot) {
        tracing::warn!(%err, "failed to restore system proxy settings left behind by a previous flproxy run");
    }
}
...
mod tests {
    ...
    /// `flproxy-api/tests/common/mod.rs`'s `temp_path` pattern.
    fn temp_dir() -> PathBuf {
        std::env::temp_dir().join(format!(
            "flproxy-sysproxy-state-test-{}",
            uuid::Uuid::new_v4()
        ))
    }
