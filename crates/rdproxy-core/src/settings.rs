//! Global proxy settings, persisted as JSON in the rdproxy data directory.
...
/// Returns the rdproxy data directory: `$RDPROXY_HOME` if set, otherwise
/// `~/.rdproxy` (using `$HOME` on Unix or `%USERPROFILE%` on Windows).
/// Creates the directory if it doesn't already exist.
pub fn data_dir() -> PathBuf {
    let dir = if let Ok(custom) = std::env::var("RDPROXY_HOME") {
        PathBuf::from(custom)
    } else {
        let home = std::env::var("HOME")
            .or_else(|_| std::env::var("USERPROFILE"))
            .unwrap_or_else(|_| ".".to_string());
        Path::new(&home).join(".rdproxy")
    };
