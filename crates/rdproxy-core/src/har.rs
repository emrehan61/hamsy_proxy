...
//! Exported entries carry an `_rdproxy` extension object
//! (`{matchedRules, modified, flowId, resourceType}`) so rdproxy-exported
//! HAR files round-trip losslessly through [`import_har`]; HAR files from
//! other tools (which lack that extension) still import, with rdproxy's
...
pub fn export_har(flows: &[Flow], creator_version: &str) -> Value {
    ...
    serde_json::json!({
        "log": {
            "version": "1.2",
            "creator": {"name": "rdproxy", "version": creator_version},
            "browser": {"name": "rdproxy", "version": creator_version},
            "pages": [],
            "entries": entries,
        }
    })
...
mod tests {
    ...
    fn export_produces_valid_har_shape() {
        ...
        assert_eq!(har["log"]["creator"]["name"], "rdproxy");
