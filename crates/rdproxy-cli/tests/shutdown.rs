//! Integration test for the signal-triggered shutdown path (`rdproxy run`
...
//! file/recovery logic (covered by `rdproxy-api`'s own unit tests), or
...
/// `tempfile`/`uuid` dependency in `rdproxy-cli`) the same way
/// `rdproxy-core/src/settings.rs`'s tests build unique temp paths.
fn fresh_temp_dir(label: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "rdproxy-cli-shutdown-test-{label}-{}-{}",
        std::process::id(),
        nanos
    ))
}
...
/// Spawns `rdproxy run` (never with `--system-proxy`), waits for it to be
...
fn run_signal_test(signal_name: &str) {
    ...
    let mut child = Command::new(env!("CARGO_BIN_EXE_rdproxy"))
        .args([
            "--data-dir",
    ...
    loop {
        match child.try_wait().expect("try_wait") {
            Some(exit_status) => {
                assert!(
                    exit_status.success(),
                    "rdproxy exited with {exit_status:?} after {signal_name}"
                );
                break;
            }
            None => {
                if Instant::now() >= deadline {
                    ...
                    panic!("rdproxy did not exit within 15s of receiving {signal_name}; stderr: {stderr}");
                }
