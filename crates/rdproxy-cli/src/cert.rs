//! `rdproxy cert` subcommands: inspecting the MITM root certificate
//! authority and (best-effort) installing/removing it from the OS trust
//! store.
//!
//! These subcommands operate directly on the on-disk CA in the rdproxy data
//! directory; there is no IPC with a running `rdproxy run` process.
...
use clap::Subcommand;
use rdproxy_proxy::CertAuthority;

use crate::resolve_data_dir;

/// `rdproxy cert` subcommands.
...
/// helper in `rdproxy_api::sysproxy`.
...
/// Windows `ROOT` store, matching by the CA's common name (`"rdproxy CA"`,
/// see `rdproxy_proxy::ca::generate_ca_pem`).
fn windows_uninstall_args() -> Vec<String> {
    vec![
        "certutil".to_string(),
        "-delstore".to_string(),
        "ROOT".to_string(),
        "rdproxy CA".to_string(),
    ]
}
...
fn linux_install_commands(cert_path: &Path) -> (String, String) {
    (
        format!(
            "cp {} /usr/local/share/ca-certificates/rdproxy-ca.crt",
            cert_path.display()
        ),
...
fn linux_uninstall_commands() -> (String, String) {
    (
        "rm /usr/local/share/ca-certificates/rdproxy-ca.crt".to_string(),
        "update-ca-certificates".to_string(),
    )
}
...
fn nss_install_command(cert_path: &Path) -> String {
    format!(
        "certutil -d sql:$HOME/.pki/nssdb -A -t \"C,,\" -n rdproxy -i {}",
        cert_path.display()
    )
}

fn linux_install_ca_file(cert_path: &Path) -> std::result::Result<(), String> {
    let dest = Path::new("/usr/local/share/ca-certificates/rdproxy-ca.crt");
...
fn install() -> Result<()> {
    ...
    if cfg!(target_os = "macos") {
        ...
        match run_command(arg_refs[0], &arg_refs[1..]) {
            Ok(_) => println!("Installed the rdproxy CA into the macOS System keychain."),
            Err(e) => {
        ...
        match linux_install_ca_file(&cert_path) {
            Ok(()) => println!("Installed the rdproxy CA into the system trust store."),
            Err(e) => {
                eprintln!("Could not install automatically ({e}).");
        ...
        let arg_refs: Vec<&str> = args.iter().map(String::as_str).collect();
        match run_command(arg_refs[0], &arg_refs[1..]) {
            Ok(_) => println!("Installed the rdproxy CA into the Windows ROOT store."),
            Err(e) => {
                eprintln!("Could not install automatically ({e}).");
                eprintln!(
...
fn uninstall() -> Result<()> {
    ...
    if cfg!(target_os = "macos") {
        ...
        let arg_refs: Vec<&str> = args.iter().map(String::as_str).collect();
        match run_command(arg_refs[0], &arg_refs[1..]) {
            Ok(_) => println!("Removed the rdproxy CA from the macOS System keychain."),
            Err(e) => {
        ...
        match linux_uninstall_ca_file() {
            Ok(()) => println!("Removed the rdproxy CA from the system trust store."),
            Err(e) => {
                eprintln!("Could not remove automatically ({e}).");
        ...
        let arg_refs: Vec<&str> = args.iter().map(String::as_str).collect();
        match run_command(arg_refs[0], &arg_refs[1..]) {
            Ok(_) => println!("Removed the rdproxy CA from the Windows ROOT store."),
            Err(e) => {
                eprintln!("Could not remove automatically ({e}).");
                eprintln!(
                    "Re-run this command from an elevated (Administrator) prompt:\n  {}",
...
mod tests {
    ...
    fn windows_uninstall_args_match_ca_common_name() {
        let args = windows_uninstall_args();
        assert_eq!(args, vec!["certutil", "-delstore", "ROOT", "rdproxy CA"]);
    }
    ...
    fn linux_commands_reference_expected_paths() {
        ...
        assert_eq!(
            cp,
            "cp /tmp/ca.pem /usr/local/share/ca-certificates/rdproxy-ca.crt"
        );
        assert_eq!(update, "update-ca-certificates");
        let (rm, update2) = linux_uninstall_commands();
        assert_eq!(rm, "rm /usr/local/share/ca-certificates/rdproxy-ca.crt");
