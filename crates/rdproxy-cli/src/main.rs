//! Command-line entry point for rdproxy: a local MITM HTTP(S) debugging
//! proxy. Wires together `rdproxy-core`, `rdproxy-proxy`, and `rdproxy-api`
//! into a runnable binary (`rdproxy run`), plus `cert`/`rules`/`proxy`
//! management subcommands that operate directly on rdproxy's on-disk state
...
/// rdproxy: a local MITM HTTP(S) debugging proxy.
///
/// Running with no subcommand is equivalent to `rdproxy run`.
#[derive(Parser, Debug)]
#[command(name = "rdproxy", version, about = "Local MITM HTTP(S) debugging proxy", long_about = None)]
struct Cli {
...
/// Resolves the rdproxy data directory: `explicit` if given, otherwise
/// [`rdproxy_core::data_dir`] (which itself honors `$RDPROXY_HOME`).
pub(crate) fn resolve_data_dir(explicit: Option<&Path>) -> PathBuf {
    match explicit {
        Some(dir) => dir.to_path_buf(),
        None => rdproxy_core::data_dir(),
    }
}
...
fn install_panic_restore_hook(data_dir: PathBuf) {
    ...
    std::panic::set_hook(Box::new(move |info| {
        match std::panic::catch_unwind(|| rdproxy_api::sysproxy_state::restore_if_marked(&data_dir))
        {
            Ok(Ok(_)) => {}
            Ok(Err(err)) => {
                eprintln!("rdproxy: failed to restore the system proxy after a panic: {err}")
            }
            Err(_) => eprintln!(
                "rdproxy: panicked again while restoring the system proxy after a panic (ignored)"
            ),
        }
        previous(info);
    }));
