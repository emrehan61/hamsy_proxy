//! `rdproxy proxy on|off|status`: OS system-proxy control.
//!
//! Operates directly on the on-disk `Settings` (for the configured proxy
//! port) via [`rdproxy_api::sysproxy`); there is no IPC with a running
//! `rdproxy run` process.
...
fn on() -> Result<()> {
    ...
    let settings = rdproxy_core::Settings::load(&data_dir.join("settings.json"));
    let bypass: Vec<String> = SYSTEM_PROXY_BYPASS.iter().map(|s| s.to_string()).collect();
    rdproxy_api::sysproxy_state::acquire(&data_dir, "127.0.0.1", settings.proxy_port, &bypass)
...
fn off() -> Result<()> {
    let data_dir = resolve_data_dir(None);
    rdproxy_api::sysproxy_state::release(&data_dir)
        .map_err(anyhow::Error::msg)
...
fn status() -> Result<()> {
    match rdproxy_api::sysproxy::status() {
