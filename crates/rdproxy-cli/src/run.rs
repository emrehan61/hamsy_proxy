//! The `run` command: wires the proxy engine ([`rdproxy_proxy`]) and the
//! REST/WebSocket API + web UI server ([`rdproxy_api`]) together into one
//! running rdproxy instance, sharing a single [`ProxyContext`] between them.
...
use rdproxy_core::{FlowStore, RulesStore, Settings};
use rdproxy_proxy::upstream::Connector;
use rdproxy_proxy::{CertAuthority, ProxyContext, ProxyServer};

use crate::hooks::{RealCertHook, RealReplayHook};
use crate::resolve_data_dir;
use crate::shutdown;

/// Options for `rdproxy run` (and the bare `rdproxy` invocation, which is
/// equivalent to `rdproxy run` with whatever flags were given at the top
/// level).
...
pub struct RunArgs {
    ...
    /// Override the rdproxy data directory (default: `$RDPROXY_HOME` or `~/.rdproxy`).
    #[arg(long = "data-dir")]
...
/// Bypass list applied when rdproxy enables the OS system proxy: traffic to
...
impl SystemProxyGuard {
    ...
    async fn restore(&self) -> Option<Result<bool, String>> {
        ...
        match tokio::task::spawn_blocking(move || {
            rdproxy_api::sysproxy_state::restore_if_marked(&data_dir)
        })
        .await
        {
...
impl Drop for SystemProxyGuard {
    fn drop(&mut self) {
        ...
        if let Err(err) = rdproxy_api::sysproxy_state::restore_if_marked(&self.data_dir) {
...
pub async fn run(args: RunArgs) -> Result<()> {
    ...
    // A previous rdproxy process may have died (crash/hard kill) without
    ...
    rdproxy_api::sysproxy_state::recover_stale(&data_dir);
    ...
    // `rdproxy-proxy/tests/common/mod.rs::spawn_proxy_trusting`.
    ...
    let api_state = rdproxy_api::ApiState::new(
        flows.clone(),
        rules.clone(),
        settings.clone(),
    ...
    let api_task = tokio::spawn({
        let shutdown = shutdown_future(shutdown_rx.clone());
        async move {
            let router = rdproxy_api::router(api_state);
    ...
    let lan_ip = rdproxy_api::lan_addresses().into_iter().next();
    println!(
        "{}",
        format_banner(
            env!("CARGO_PKG_VERSION"),
    ...
    if system_proxy_requested {
        ...
        match rdproxy_api::sysproxy_state::acquire(&data_dir, "127.0.0.1", proxy_port, &bypass) {
    ...
    match system_proxy_guard.restore().await {
        ...
        Some(Err(err)) => {
            ...
            println!("  Warning: failed to restore the system proxy automatically ({err}); run `rdproxy proxy off` to fix it manually.");
        }
    ...
    tracing::info!("rdproxy stopped, goodbye");
    Ok(())
}
...
fn format_banner(
    ...
    out.push_str(&format!(
        "  CA cert    http://{display_ip}:{ui_port}/cert/rdproxy-ca.crt\n"
    ));
...
mod tests {
    ...
    fn banner_includes_lan_ip_when_present() {
        ...
        assert!(banner.contains("rdproxy 0.1.0"));
        assert!(banner.contains("http://127.0.0.1:9080"));
        assert!(banner.contains("http://192.168.1.42:9081/cert/rdproxy-ca.crt"));
    ...
    fn banner_falls_back_without_lan_ip() {
        ...
        assert!(banner.contains("http://127.0.0.1:9081/cert/rdproxy-ca.crt"));
