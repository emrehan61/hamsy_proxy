...
/// Discover an already-running viewer without starting/stopping it or writing
/// any files. Authenticate its remembered port before the agent reads sessions.
pub(crate) async fn agent_viewer_url(data_dir: &Path) -> Result<Option<String>> {
    let dir = data_dir.join("har-viewer");
    if !dir.exists() {
        return Ok(None);
    }
    ...
    Ok(Some(format!("http://127.0.0.1:{}", d.port)))
}
...
fn router(viewer: Arc<Viewer>, dir: &Path) -> Router {
    ...
    let reads = Router::new()
        .merge(hamsy_api::sessions::router())
