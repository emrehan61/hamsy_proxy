...
//! runs `rdproxy-core` rules against it, dispatches to the upstream server,
...
//! never silently skipped, without needing to leave `rdproxy-core` or
...
pub(crate) async fn dispatch(
    ...
    let timings = rdproxy_core::Timings {
        blocked: 0.0,
        dns: -1.0,
        connect: connect_ms,
        ssl: ssl_ms,
...
async fn handle_captured_request(
        ...
        ReqBody::Streamed(state) if outcome.body.is_none() => {
            ...
            let finalize = move || {
                ...
                if let Some(summary) = flows.update(flow_id, |f| {
                    ...
                    let _ = events.send(rdproxy_core::ServerEvent::Flow { flow: summary });
                }
            };
    ...
    let finalize = move || {
        ...
        if let Some(summary) = flows_for_finalize.update(flow_id, |f| {
            ...
            let _ = events_for_finalize.send(rdproxy_core::ServerEvent::Flow { flow: summary });
        }
    };
...
async fn respond_mocked(
    ...
    let resp_record = ResponseRecord {
        ...
        body: rdproxy_core::to_payload(&mocked.body, content_type.as_deref(), None, max_body_bytes),
    };
    let finished_at = now_ms();
    if let Some(summary) = ctx.flows.update(flow_id, |f| {
        f.original_request = original_request.clone();
...
mod tests {
    use super::*;
    use rdproxy_core::{BodyCond, BodyCondOp, Matcher};
