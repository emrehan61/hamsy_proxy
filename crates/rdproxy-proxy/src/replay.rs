...
/// [`rdproxy_core::FlowSummary::client_addr`] to `"replay"` - there is no
/// dedicated boolean field on `Flow`/`FlowSummary` for this, so the task
/// spec calls for repurposing `client_addr` as the marker.
pub async fn replay(ctx: &ProxyContext, req: rdproxy_core::RequestRecord) -> Result<FlowId> {
...
async fn run_replay(
    ctx: &ProxyContext,
    flow_id: FlowId,
    req: rdproxy_core::RequestRecord,
    url: url::Url,
    conn_info: ConnInfo,
) -> Result<()> {
