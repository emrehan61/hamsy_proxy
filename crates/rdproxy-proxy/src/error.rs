//! Shared error type for the `rdproxy-proxy` crate.

use thiserror::Error;

/// Errors that can occur anywhere in `rdproxy-proxy`.
///
...
pub enum ProxyError {
    ...
    /// Wraps an `rdproxy-core` failure (rule/body/settings logic).
    #[error("core error: {0}")]
    Core(#[from] rdproxy_core::CoreError),
