//! `rdproxy-proxy`: a from-scratch MITM HTTP(S) debugging proxy engine.
//!
//! Built directly on `tokio`/`hyper`/`rustls` (not on an all-in-one proxy
//! framework), this crate implements the network side of rdproxy: accepting
//! client connections, terminating and re-originating TLS for MITM'd HTTPS
//! traffic, applying `rdproxy-core` rules to requests/responses, recording
//! [`rdproxy_core::Flow`]s, and relaying WebSocket traffic.
//!
//! Module map:
